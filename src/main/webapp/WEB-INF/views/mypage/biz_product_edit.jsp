...
<head>
 ...
 $(document).ready(function(){
	 ...
	 var html = '';
	 <c:forEach items="${product.pdList}" var="opt">
	  html += '<tr>';
	  html += '<td><input type="text" name="pd_name" class="form-control item_name" value="${opt.pd_name}"/></td>';
	  html += '<td><input type="text" name="pd_inven" class="form-control item_quantity" value="${opt.pd_inven}"/></td>';
	  html += '<td><input type="text" name="pd_price" class="form-control item_price" value="${opt.pd_price}"/></td>';
		 
	  html += '<td><button type="button" class="btn btn-danger btn-sm remove"><span class="glyphicon glyphicon-minus"></span></button></td></tr>';
	  $('#item_table').append(html);
	 </c:forEach>
	 
	var tagString = '${product.tag}';
	var tags = tagString.split("#");
	for(var i in tags) {
		$(".mytag").eq(i-1).val(tags[i]);
	}
            ...
            <div class="card-body">
              <table class = "table export-table">
                <tr class = "export-r">
 				 ...
 				 <option value="cosmetic" <c:if test="${product.category eq 'cosmetic'}">selected</c:if> >Cosmetic</option>
 				 <option value="pet" <c:if test="${product.category eq 'pet'}">selected</c:if>>Pet product</option>
 				 <option value="food" <c:if test="${product.category eq 'food'}">selected</c:if>>Food</option>
 				 <option value="etc" <c:if test="${product.category eq 'etc' }">selected</c:if>>Etc</option>
					</select> </td>
                </tr>
                <tr class = "export-r">
                ...
                <td class = "p-c2">   
                      ...
                      <input type="text" class="form-control form-control-user col-6" id="exampleInputPassword" name="name" value="${product.name }" placeholder="(직접 입력하세요)"> 
                ...
                <input type="text" class="form-control form-control-user col-6" id="exampleInputPassword" name="des" value="${product.des}" placeholder="(직접 입력하세요)"> 
                ...
                <tr class = "export-r">
                  ...
                  <td class = "p-c2">
                   ...
                   <input type="text" class="form-control form-control-user col-4" id="exampleInputPassword" name="price" value="${product.price}" placeholder="(숫자만 입력하세요)"> 
