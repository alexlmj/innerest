                ...
                <div class="p-3 p-lg-5 border">
                  <table class="table site-block-order-table mb-5">
                    ...
                    <c:forEach items="${cpList}" var="p">
                       <c:set var= "price" value="${p.quantity*p.option.pd_price}"/>
                   	   <tr>
                        <td>${name} (옵션:${p.option.pd_name})<strong class="mx-2">x</strong> ${p.quantity}</td>
                        <td class="pd_price">${price}</td>
                      </tr>
                      <c:set var= "sum" value="${sum + price}"/>
                      <input type="hidden" name="pd_num" value="${p.option.pd_num}">
 ...
 <jsp:include page="/WEB-INF/views/common/main-footer.jsp"></jsp:include>
  ...
  $(document).ready(function(){
	 $("#orderBtn").click(function(){
		...
		$("#checkoutForm").attr("action","/shop/checkout_s"); 
